...
class Reference:
    ...
    def get_sub_references(self, embedded_queries, a):
        ...
        for embedded_query in embedded_queries:
            ...
            # 🔹 보조 참조 인덱스 추출 (-1은 제외)
            sub_refs = [int(self.QA_list[idx][0]) for idx in valid_indices if self.QA_list[idx][0] >= 0]
            sub_refs_all.append(sub_refs)

        print(sub_refs_all)  # 디버깅용 출력
    ...
    def get_reference(self, embedded_query, k):
        ...
        🔹 보조 참조(sub_ref)는 List[List[int]]로 받아 처리합니다.
        ...
        # 🔹 보조 참조(sub_ref) (a > 0 시 작동)
        a = 2
        sub_ref_passages = self.get_sub_references(embedded_query, a)  # List[List[int]]

        final_refs_passages = []

        for i in range(n):
            main_ref = [int(x) for x in main_ref_passages[i]]
            sub_ref = sub_ref_passages[i]

            # 🔸 -1 제거 및 중복 방지를 위한 집합 구성
            final_passages = {ref for ref in sub_ref if ref >= 0}

            # 🔸 부족한 개수만큼 main_ref에서 추가
            for ref in main_ref:
                final_passages.add(ref)
        ...
        self.save_Q_past()
        print(final_refs_passages)  # ✅ 디버깅용 출력
